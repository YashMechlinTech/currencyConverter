...
function App() {
  ...
  const swap = () => {
    setFrom(to);
    setTo(from);
    setConvertedAmount(amount); // swap logic
  ...
  const convert = () => {
    if (currencyinfo[to]) {
      ...
      console.error(`Currency data for ${to} is not available.`);
    }
  };
          ...
          >
            <div className="w-full mb-1">
              <Input
                label="From"
                amount={amount}
                OnAmountChange={(amount) => setAmount(amount)} // Correct prop name
                OnCurrencyChange={(currency) => setFrom(currency)} // Correct prop name
            ...
            <div className="w-full mt-1 mb-4">
              <Input
                ...
                OnAmountChange={() => {}} // No-op for To input
                OnCurrencyChange={(currency) => setTo(currency)}
                currencyOptions={options}
                selectCurrency={to}
                amountDisable // Disable amount input for To field
                currencyDisable={false} // Allow selection in the dropdown
              />
            </div>
            <button
              type="submit"
